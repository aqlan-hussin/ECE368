 ...
 *********************************************************/
 link NEW(int w, link next, int wt) {
...
void GRAPHinsertE(Graph G, Edge e) {
    int v = e.v;
    int w = e.w;
    int d = POINTdist(G->points[v], G->points[w]);
 ...
 *********************************************************/
 void GRAPHshowsp(Graph G) {
    ...
    for (v = G->s; v != G->d; v = temp){
       printf("%d ", v);
       for (w = G->d; w != v; w = G->pred[w]){ 
          if(w == v) {break;}
          temp = w;   
        }
     }
 ...
 *********************************************************/
 Graph GRAPHscan(int V, int E, FILE* file1, FILE* file2) {
    ...
    //fscanf(file1, "%d %d", &V, &E);

    /*for(i=0; i<v; i++) {
      fscanf(file1, "%d %d %d", &p[i]->index, &p[i]->x, &p[i]->y);
    }*/
